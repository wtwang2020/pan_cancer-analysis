...
data <- data_fpkm %>%
  group_by(project) %>%
  summarize(across(gene, median, na.rm = TRUE))
data <- fpkm_function(data,reverse = TRUE)
#write.table(data,file = "纯TCGA肿瘤数据(中位数).csv",sep = ",",row.names = FALSE)
write.table(data,file = "纯TCGA肿瘤数据配体(中位数).csv",sep = ",",row.names = FALSE)
